...
// Copyright (c) 2014 The Saffroncoin developers
...
const string strMessageMagic = "Saffroncoin Signed Message:\n";
...
bool ConnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (GetValueOut(block.vtx[0]) > GetBlockValue(pindex->nHeight, nFees, prevHash))
        return state.DoS(100, error("ConnectBlock() : coinbase pays too much (actual=%"PRI64d" vs limit=%"PRI64d")", GetValueOut(block.vtx[0]), GetBlockValue(pindex->nHeight, nFees, prevHash)));
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SaffroncoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SaffroncoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("Saffroncoin miner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
